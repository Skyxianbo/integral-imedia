...
<template lang="html">
    <v-list :source.sync="source" :count.sync="count" :search="currentSearch" :list.sync="commendList">
        <div slot="list-header">
            <span>推荐管理</span>
            <span class="limit-alert" v-if="source == 2">您还可上线 {{limit-total}}/{{limit}} 条推荐</span>
        </div>
        ...
        <div slot="list-table">
            <div class="table-panel" :class="itemSort? 'z-index-1000': ''">
                <table class="table">
                    <thead>
                        <tr>
                    ...
                    <tbody id="sortableItems">
                        <tr class="sortable" v-for="item in commendList" data-value="{{$index}}">
                            ...
                            <td>{{item.origin_type == 1? '电视台' : '电视台'}}</td>
                            <td>{{item.type_str}}</td>
                            <td>{{item.pv}}</td>
                            <td v-if="source == 2">
                                <div v-if="!itemSort">
...
export default {
    ...
    methods: {
        getList(type) {
            ...
            this.requestApi('/mcommend/get_list', param, (response) => {
                this.commendList = this.initList(response.data.list);
                this.limit = response.data.commend_param;
                this.total = response.data.commend_count_up;
        ...
        changeStatus(type, item) {
            if (type == 0) {
                ...
                layer.msg(`确定${this.status_str[status]}该推荐位吗？`, {
                    ...
                    yes: (index) => {
                        layer.close(index);
                        this.requestApi('/mcommend/status/' + item.id, {
                            status: status
                        }, (response) => {
                ...
                layer.msg(`确定批量${this.status_str[type]}选中推荐位吗？`, {
                    ...
                    yes: (index) => {
                        ...
                        this.requestApi('/mcommend/status/' + id, {
                            status: type
                        }, (response) => {
        ...
        sortItemSave() {
            ...
            this.requestApi('/mcommend/set_order', {
                data: param
            }, (response) => {
                layer.msg("已重新进行排序");
